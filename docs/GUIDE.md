...
- The local Hook server listens only on `127.0.0.1`, never on a LAN or public interface.
...
The desktop core uses Electron and local files without a frontend framework. The optional companion has a separate Cloudflare Worker/D1 relay; see [companion setup](REMOTE_SETUP.md).
...
reply buttons. See [build and setup](REMOTE_SETUP.md),
[decisions](REMOTE_DECISIONS.md), [protocol](REMOTE_PROTOCOL.md) and
...
See [docs/RELEASING.md](RELEASING.md) for the default unsigned release procedure and the optional SignPath configuration.
...
- The companion supports paired remote approvals only with explicit PC-local authorization. Its separate LAN listener does not change the loopback-only Hook server.
