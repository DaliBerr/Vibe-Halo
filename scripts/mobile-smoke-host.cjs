...
(async () => {
  ...
  await remote.initialize(); await remote.configure({ relayOrigin: "http://127.0.0.1:8787", enrollmentCode: code, name: "Synthetic PC" });
  remote.setControl(true); await remote.beginPairing();
  const bridgeToken = crypto.randomBytes(32).toString("hex"), outputs = [];
  const fixture = { relayOrigin: "http://127.0.0.1:8787", bridgeOrigin: "http://127.0.0.1:8788", bridgeToken, code: remote.pairing.code, pcId: remote.pcId, lanPort: remote.lan?.port };
