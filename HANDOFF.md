...
Vibe Halo is a Windows-first Electron dynamic-island interface for AI coding clients. Version 0.2.0 introduces a shared adapter registry and 19 integrations while retaining one window, one global approval FIFO, loopback-only transport, strict renderer isolation, and fail-open behavior.

- Dynamic-island approvals: Codex, ZCode, Qwen Code, Copilot CLI, Claude Code, CodeBuddy, Hermes, and OpenCode.
- Exact interactive answers: Claude/CodeBuddy Elicitation and Hermes clarify.
...
- Windows unpacked build: 0.2.0 launches, creates a valid process-owned runtime identity, and loads all unpacked managed assets.
- Codex: trusted Hook health detected; command Hook single-allow path returned the exact Codex protocol.
- ZCode 3.3.0: auto-detected, exact native hook schema installed, allow/deny/close/offline outputs verified, and disabled-by-user persistence plus re-enable verified.
...
- Artifact: `dist/Vibe-Halo-Setup-0.2.0-x64.exe`.
