...
Vibe Halo is a Windows-first Electron dynamic-island interface for AI coding clients. Version 0.4.0 adds live English and Simplified Chinese localization plus a bilingual NSIS installer while retaining the gated updater, centered mini-island, one window, one global approval FIFO, loopback-only transport, strict renderer isolation, and fail-open behavior.
...
- `src/i18n.js`: complete `en-US`/`zh-CN` catalogs, Windows locale resolution, bounded interpolation, and renderer string projection.
...
- Automated suite: 106 tests covering 19 adapters, codecs, forms, global FIFO, server authentication/bounds, command-hook E2E, offline fallback, installers, backups, idempotence, explicit disables, safe uninstall, IPC, sizing, stores, localization, settings migration, and legacy regressions.
...
- Windows unpacked build: 0.4.0 launched and exited cleanly in isolated English and Simplified Chinese smoke profiles. Both expanded-island renders were visually checked, the isolated Chinese approval click resolved successfully, and the package contains LICENSE/NOTICE, both READMEs, and the localization module.
- Windows installer: bilingual `en_US`/`zh_CN` NSIS configuration built successfully without a language selector. `Vibe-Halo-Setup-0.4.0-x64.exe` is intentionally unsigned and update-disabled; SHA-256 is `F06741253A30FD79581DCD3DA9F2F420BC4D9C32DE5062F428AB58C1BEF78D30`.
...
- Artifact: `dist/Vibe-Halo-Setup-0.4.0-x64.exe`.
