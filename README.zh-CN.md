...
> [!WARNING]
> 标记为 **Unsigned Preview（未签名预览版）** 的安装包仅供测试。Windows SmartScreen 可能显示“未知发布者”；macOS 需要在 Finder 中右键应用并选择“打开”以批准未公证应用。所有预览包都关闭自动更新。
...
预览版未使用 Apple Developer ID 签名，也未公证。从 DMG 将应用拖入“应用程序”后，请在 Finder 中按住 Control 点击或右键 Vibe Halo，选择“打开”并确认一次。Vibe Halo 以 accessory 应用运行，不显示 Dock 图标，也不申请辅助功能或屏幕录制权限。
