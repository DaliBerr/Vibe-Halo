...
- **可选本地历史**：仅从托盘打开的右侧面板会保存有界的审批、问答和 Codex 计划详情，不在桌面留下永久入口。
...
- Codex 在计划模式下结束一轮任务时，Vibe Halo 会显示专门的“计划已就绪”通知，并在可用时展示完整计划内容。
- 完成通知默认显示 8 秒；新的提示或审批会优先展示并清理旧通知。
- UI 优先级固定为：**审批/精确交互 > 等待输入 > 完成通知**。

### 最近事件

- 从托盘打开“最近事件”，会显示一个独立的 460 × 720 右侧面板；顶部实时灵动岛保持独立，并始终位于历史面板上方。
...
- Codex “计划已就绪”正文会保存；普通任务完成通知明确不进入历史。
