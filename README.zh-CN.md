...
**面向 Windows 的 Codex、ZCode、Claude Code、OpenCode 权限审批弹窗与灵动岛通知工具。**
...
> [!NOTE]
> **实机验证范围：**Codex 与 ZCode 已在维护者环境完成真实客户端往返测试。其他适配器具备自动契约测试，但仍可能存在客户端特有的兼容性 Bug；评估这些集成时，请保留客户端原生审批入口。

> [!IMPORTANT]
> Vibe Halo 是 [Clawd on Desk](https://github.com/rullerzhou-afk/clawd-on-desk) 的二次开发与独立维护分支，不是上游官方版本。项目保留并改造了上游的 Hook、插件、审批传输和生命周期设计，移除了桌宠、主题、会话状态动画、远程审批与移动端能力，将产品重心收敛为一个 Windows 顶部灵动岛。上游版权与贡献归属见 [NOTICE.md](NOTICE.md)。

Vibe Halo 在需要你注意时出现在当前工作显示器顶部：审批请求可以直接允许或拒绝，支持稳定回答协议的问题可以在岛内作答，任务结束时显示短暂通知。没有明确决定、应用不可用或协议异常时，请求会安全交回原客户端，不会自动允许或拒绝。

## 快速开始

1. 从 [GitHub Releases](https://github.com/DaliBerr/Vibe-Halo/releases) 下载 `Vibe-Halo-Setup-<version>-x64.exe`。
2. 启动 Vibe Halo，从托盘打开“客户端集成”，检查本机客户端检测结果与集成健康状态。
3. 如果使用 Codex，请先在 Codex 输入 `/hooks`，审核已安装的 Vibe Halo command Hook，再触发一次审批。

安装器内置英文和简体中文资源，并自动跟随 Windows 显示语言。应用默认同样跟随 Windows，也可以从托盘“语言”中即时切换，无需重启。

> [!WARNING]
> 标记为 **Unsigned Preview（未签名预览版）** 的安装包仅供测试。Windows SmartScreen 可能显示“未知发布者”，在签名稳定版可用前，自动更新保持关闭。
...
- 只有发布者签名匹配的 Windows 正式构建才启用自动更新。
...
如果 Releases 页面暂时没有签名安装包，说明公开签名发布链尚未就绪；不要把来源不明的未签名安装包当作正式版本。
...
安装器会根据 Windows 显示语言自动选择英文或简体中文，不额外弹出语言选择框。应用启动后可独立切换语言，不会修改 Windows 设置。
...
- Windows x64；主要人工验收环境为 Windows 11。
...
| `开机启动` | 控制 Windows 登录后启动 Vibe Halo |
...
| `语言` | 选择“跟随 Windows”、English 或“简体中文”；当前窗口和排队项目会立即刷新 |
...
NSIS 卸载程序也会调用相同清理流程。清理只删除 Vibe Halo 自己的 Hook/插件记录；第三方配置、首次备份以及应用用户数据会保留。
...
| Windows 集成 | `koffi`、系统托盘、NSIS x64 |
| 自动更新 | `electron-updater`、GitHub Releases、Authenticode 验证 |
...
| `npm run build:dir` | 生成未安装的 `dist/win-unpacked` |
| `npm run build` | 生成本地 x64 NSIS 安装包，不发布、不启用更新 |
...
- Renderer IPC、动态按钮、问题表单、窗口尺寸、阴影边界和多屏定位纯逻辑。
- Hook mock-server 端到端与离线回退。
- SignPath 外部签名暂存、元数据生成和发布配置。

以下内容仍需 Windows 人工验收：透明窗口、阴影、焦点、动画、多屏/DPI、真实客户端 Hook 回传、托盘菜单、NSIS 安装/卸载及已签名更新。
...
### Vibe Halo 是 Vibe Island 的 Windows 替代方案吗？

Vibe Halo 在 Windows 上提供类似的顶部审批与通知工作流，但它是独立维护的开源项目，并非 Vibe Island 的官方 Windows 版本。只有在 AI 编程客户端需要支持的权限决定、提出支持的交互问题或完成任务时，它才会出现。
...
源码运行、本地打包和未签名构建会有意禁用自动更新。只有发布者签名通过验证的官方 Windows 构建才显示更新控制。

## 已知边界

- 当前发布目标是 Windows x64；不提供 macOS、Linux 或 ARM64 包。
- 应用和安装器支持英文与简体中文；繁体中文 Windows 当前自动使用英文，也可手动选择简体中文。
...
- 当前只有 Codex 与 ZCode 完成真实客户端端到端验证；其他客户端即使契约测试通过，也可能存在尚未发现的兼容性 Bug。
...
5. 涉及窗口、Hook 或 NSIS 时，在说明中记录 Windows 人工验收结果。
